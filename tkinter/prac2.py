...
def add_model_name():
    displayModelName.configure(text = inputModelName.get()+' - '+number.get()+' -CheckBox '+str(checkVar.get()))
    # file_name = inputModelName.get()
    # with open(file_name) as file:
    #     f = file.read()
    
    # displayModelName.configure(text = f)
...
# Check Box
checkVar = tk.IntVar()
inputCheckButton = tk.Checkbutton(win,text='Create Model',variable=checkVar)
inputCheckButton.select()
inputCheckButton.grid(column=0,row=3,sticky=tk.W)

# Check Box
checkVar = tk.IntVar()
inputCheckButton = tk.Checkbutton(win,text='Create Model',variable=checkVar,state='disabled')
inputCheckButton.select()
inputCheckButton.grid(column=0,row=4)
